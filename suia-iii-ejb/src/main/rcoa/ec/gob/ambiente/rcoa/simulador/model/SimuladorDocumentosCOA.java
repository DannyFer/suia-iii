...
@Entity
@Table(name = "documents_coa", schema = "coa_simulator")
@AttributeOverrides({
...
@NamedQueries({
@NamedQuery(name="SimuladorDocumentosCOA.findAll", query="SELECT d FROM DocumentosCOA d"),
@NamedQuery(name=SimuladorDocumentosCOA.GET_POR_TIPO_Y_ID, query="SELECT d FROM DocumentosCOA d where d.tipoDocumento.id = :idTipo and d.idTabla = :id and d.estado = true order by d.id DESC")})
@Filter(name = EntidadBase.FILTER_ACTIVE, condition = "docu_status = 'TRUE'")
public class SimuladorDocumentosCOA extends EntidadAuditable {
	...
	private static final String PAQUETE = "ec.gob.ambiente.rcoa.simulador.model.";
