...
@ManagedBean
@ViewScoped
public class ObservacionesEmisionLicenciaController {
	
	private static final Logger LOG = Logger.getLogger(ObservacionesEmisionLicenciaController.class);
    ...
    @Getter
    @Setter
    private Map<String, ObservacionesResolucionAmbientalBean> listaObservacionesBB;
    @Getter
    @Setter
    ...
    @Getter
    @Setter
    private Map<String, ObservacionesResolucionAmbientalBean> listaObservacionesH;
    ...
    public String obtenerIdSeccion(String idFormulario) {
    	ObservacionesResolucionAmbientalBean observacion = listaObservacionesBB.get(idFormulario);
        String sec = "";
    ...
    public void cargarObservaciones(String idFormulario, String nombreClase, Integer idClase, String... secciones) {
        try {
        	ObservacionesResolucionAmbientalBean observacion = new ObservacionesResolucionAmbientalBean();
            observacion.setNombreClase(nombreClase);
            observacion.setIdClase(idClase);
            observacion.setSeccion(secciones);

            observacion.setListaObservaciones(observacionesResolucionAmbientalFacade.listarPorIdClaseNombreClase(idClase, nombreClase, secciones));
            if (observacion.getListaObservaciones() != null && !observacion.getListaObservaciones().isEmpty()) {
                for (ObservacionesResolucionAmbiental ob : observacion.getListaObservaciones()) {
            ...
            for (ObservacionesResolucionAmbiental ob : observacion.getListaObservaciones()) {
                List<ObservacionesResolucionAmbiental> elementos = new ArrayList<>();
            ...
            for (String seccion : secciones) {
                if (!observacion.getMapaSecciones().containsKey(seccion) && !seccion.equals("*")) {
                    observacion.getMapaSecciones().put(seccion, new ArrayList<ObservacionesResolucionAmbiental>());
                }
            }
     ...
     * @param seccion
     */
    public void reasignarIndice(ObservacionesResolucionAmbientalBean observacionesBB, String seccion) {
    	if (observacionesBB.getMapaSecciones() != null && !observacionesBB.getMapaSecciones().isEmpty()) {
    		if (observacionesBB.getMapaSecciones().get(seccion) != null && !observacionesBB.getMapaSecciones().get(seccion).isEmpty()) {
    			int i = 0;
    			for (ObservacionesResolucionAmbiental ob : observacionesBB.getMapaSecciones().get(seccion)) {
    				ob.setIndice(i);
    				i++;
    			}
    		}
    ...
    public void agregarObservacion(String idFormulario, String idClase, String nombreClase, String seccion) {
        ObservacionesResolucionAmbiental ob = new ObservacionesResolucionAmbiental();
        ob.setCampo("");
        ob.setDescripcion("");
        ...
        if (!listaObservacionesBB.get(idFormulario).getMapaSecciones().containsKey(seccion)) {
            listaObservacionesBB.get(idFormulario).getMapaSecciones().put(seccion, new ArrayList<ObservacionesResolucionAmbiental>());
        }
    ...
    public void guardar(String idFormulario, String seccion) {
        try {
        	observacionesResolucionAmbientalFacade.guardar(listaObservacionesBB.get(idFormulario).getMapaSecciones().get(seccion));
    ...
    public void eliminar(String idFormulario, String seccion, ObservacionesResolucionAmbiental observacionesInventarioForestal) {
    	ObservacionesResolucionAmbiental obj = listaObservacionesBB.get(idFormulario).getMapaSecciones().get(seccion).get(observacionesInventarioForestal.getIndice());
        if (obj.getId() != null) {
            obj.setEstado(false);
    ...
    public ObservacionesResolucionAmbientalBean getObservacionesBB() {
        return getObservacionesBB("id");
    }
    
    public ObservacionesResolucionAmbientalBean getObservacionesBB(String idFormulario) {
        if (listaObservacionesBB.containsKey(idFormulario)) {
            return listaObservacionesBB.get(idFormulario);
        }
        return new ObservacionesResolucionAmbientalBean();
    }
    
    public Boolean validarObservaciones(Usuario usuario, Boolean estado, String seccion,String idComponete) {
        List<ObservacionesResolucionAmbiental> observaciones = this.getObservacionesBB(idComponete).getMapaSecciones().get(seccion);
    ...
    public Boolean validarObservaciones(Usuario usuario, Boolean estado, String seccion) {
        List<ObservacionesResolucionAmbiental> observaciones = this.getObservacionesBB().getMapaSecciones().get(seccion);

        return validarObservaciones(usuario,  estado, seccion, observaciones );
    }
    
    public Boolean validarObservaciones(Usuario usuario, Boolean estado, String seccion,List<ObservacionesResolucionAmbiental> observaciones ) {
        List<ObservacionesResolucionAmbiental> observacionesPendientes = new ArrayList<>();
        if (estado) {
            for (ObservacionesResolucionAmbiental observacion : observaciones) {
        ...
        if (observacionesPendientes.size() > 0) {
            try {
            	observacionesResolucionAmbientalFacade.guardar(observacionesPendientes);
            } catch (ServiceException e) {
                LOG.error(e);
    ...
    public Boolean esNuevoComentario(ObservacionesResolucionAmbiental ob) {
    ...
    public void cargarObservacionesHistorial(String idFormulario, String nombreClase, Integer idClase, Date fechaRegistro, String... secciones) {
        try {
			...
			observacion.setListaObservaciones(observacionesResolucionAmbientalFacade.listarHistorialPorIdClaseNombreClase(idClase, nombreClase,fechaRegistro, secciones));
			if (observacion.getListaObservaciones() != null
					&& !observacion.getListaObservaciones().isEmpty()) {
				for (ObservacionesResolucionAmbiental ob : observacion
    ...
    public void cargarObservacionesHistorialAnterior(String idFormulario, String nombreClase, Integer idClase, Date fechaRegistro, String... secciones) {
        try {
			...
			observacion.setListaObservaciones(observacionesResolucionAmbientalFacade.listarHistorialPorIdClaseNombreClaseHistorico(idClase,nombreClase, fechaRegistro, secciones));
			if (observacion.getListaObservaciones() != null	&& !observacion.getListaObservaciones().isEmpty()) {
				for (ObservacionesResolucionAmbiental ob : observacion.getListaObservaciones()) {
					if (ob.getIdUsuario().equals(JsfUtil.getLoggedUser().getId())) {
						ob.setDisabled(false);
    ...
    public String obtenerIdSeccionHistorico(String idFormulario) {
        ObservacionesResolucionAmbientalBean observacion = listaObservacionesH.get(idFormulario);
        String sec = "";
        for (String s : observacion.getSeccion()) {
            sec += "-" + s;
        }
