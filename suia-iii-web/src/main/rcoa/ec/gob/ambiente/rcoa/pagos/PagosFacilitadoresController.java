...
import ec.gob.ambiente.rcoa.participacionCiudadana.controller.UtilPPC;
...
@ManagedBean
@ViewScoped
public class PagosFacilitadoresController {
	...
	public void initData() throws Exception {		
		...
		if(proyectoFacilitadorPPC.getId() != null) {
			...
			if(facilitadorAdicional) {
				if(proyectoFacilitadorPPC.getValorTotalFacilitadoresAdicionales() == null)
					buscarFacilitadores = true;
				else
					valorTotalFacilitador = proyectoFacilitadorPPC.getValorTotalFacilitadoresAdicionales();
			} else {
		...
		if(buscarFacilitadores) {
			listaUsuariosFacilitadores = utilPPC.getUsuariosfacilitadores(numeroFacilitadores,proyectosBean.getProyectoRcoa());
		...
		{
			if(facilitadorAdicional && proyectoFacilitadorPPC.getValorTotalFacilitadoresAdicionales() == null)
			{
				proyectoFacilitadorPPC.setFacilitadoresAdicionales(facilitadorAdicional);
				proyectoFacilitadorPPC.setNumeroFacilitadoresAdicionales(numeroFacilitadores);
				proyectoFacilitadorPPC.setValorTotalFacilitadoresAdicionales(valorTotalFacilitador);
				proyectoFacilitadorPPC=proyectoFacilitadorPPCFacade.guardar(proyectoFacilitadorPPC);
	...
	public void uploaDocumentoPago(FileUploadEvent event)
	{
	...
	public void completarTarea(){
		try {
            ...
            params.put("facilitadorAdicional", facilitadorAdicional);
	...
	private void calcularValorPago() throws Exception {
		...
		ProyectoLicenciaCoaUbicacion ubicacionProyecto = proyectoLicenciaCoaUbicacionFacade.ubicacionPrincipal(proyectosBean.getProyectoRcoa());
		...
		CatalogoGeneralCoa valorIvaS = catalogoCoaFacade.obtenerCatalogoPorTipoYOrden(CatalogoTipoCoaEnum.TIPO_VALOR_IVA, 1);
		double valorIva = Double.parseDouble(valorIvaS.getValor()); 
		valorIva = 1 + (valorIva /100);
		
		if(codeProvincia.equals(codeGalapagos)) {
			//Si el tramite se desarrolla en la provincia de galapagos
