...
import ec.gob.ambiente.rcoa.emisionLicenciaAmbiental.facade.ObservacionesResolucionLicenciaFacade;
...
@ManagedBean
@ViewScoped
public class ObservacionesResolucionLicenciaController implements Serializable {

	private static final long serialVersionUID = 1L;
    private static final Logger LOG = Logger.getLogger(AsignacionMasivaController.class);
    @EJB
    private ObservacionesResolucionLicenciaFacade observacionesResolucionLicenciaFacade;
    
    @Getter
    @Setter
    private Map<String, ObservacionesResolucionLicenciaBean> listaObservacionesBB;
    @Getter
    @Setter
    ...
    @Getter
    @Setter
    private Map<String, ObservacionesResolucionLicenciaBean> listaObservacionesH;
    ...
    public String obtenerIdSeccion(String idFormulario) {
    	ObservacionesResolucionLicenciaBean observacion = listaObservacionesBB.get(idFormulario);
        String sec = "";
    ...
    public void cargarObservaciones(String idFormulario, String nombreClase, Integer idClase, String... secciones) {
        try {
        	ObservacionesResolucionLicenciaBean observacion = new ObservacionesResolucionLicenciaBean();
            observacion.setNombreClase(nombreClase);
            observacion.setIdClase(idClase);
            observacion.setSeccion(secciones);

            observacion.setListaObservaciones(observacionesResolucionLicenciaFacade.listarPorIdClaseNombreClase(idClase, nombreClase, secciones));
            if (observacion.getListaObservaciones() != null && !observacion.getListaObservaciones().isEmpty()) {
                for (ObservacionResolucionLicencia ob : observacion.getListaObservaciones()) {
            ...
            for (ObservacionResolucionLicencia ob : observacion.getListaObservaciones()) {
                List<ObservacionResolucionLicencia> elementos = new ArrayList<>();
            ...
            for (String seccion : secciones) {
                if (!observacion.getMapaSecciones().containsKey(seccion) && !seccion.equals("*")) {
                    observacion.getMapaSecciones().put(seccion, new ArrayList<ObservacionResolucionLicencia>());
                }
            }
     ...
     * @param seccion
     */
    public void reasignarIndice(ObservacionesResolucionLicenciaBean observacionesBB, String seccion) {
    	if (observacionesBB.getMapaSecciones() != null && !observacionesBB.getMapaSecciones().isEmpty()) {
    		if (observacionesBB.getMapaSecciones().get(seccion) != null && !observacionesBB.getMapaSecciones().get(seccion).isEmpty()) {
    			int i = 0;
    			for (ObservacionResolucionLicencia ob : observacionesBB.getMapaSecciones().get(seccion)) {
    				ob.setIndice(i);
    				i++;
    			}
    		}
    ...
    public void agregarObservacion(String idFormulario, String idClase, String nombreClase, String seccion) {
        ObservacionResolucionLicencia ob = new ObservacionResolucionLicencia();
        ob.setCampo("");
        ob.setDescripcion("");
        ...
        if (!listaObservacionesBB.get(idFormulario).getMapaSecciones().containsKey(seccion)) {
            listaObservacionesBB.get(idFormulario).getMapaSecciones().put(seccion, new ArrayList<ObservacionResolucionLicencia>());
        }
    ...
    public void guardar(String idFormulario, String seccion) {
        try {
        	observacionesResolucionLicenciaFacade.guardar(listaObservacionesBB.get(idFormulario).getMapaSecciones().get(seccion));
    ...
    public void eliminar(String idFormulario, String seccion, ObservacionResolucionLicencia ObservacionResolucionLicencia) {
    	ObservacionResolucionLicencia obj = listaObservacionesBB.get(idFormulario).getMapaSecciones().get(seccion).get(ObservacionResolucionLicencia.getIndice());
        if (obj.getId() != null) {
            obj.setEstado(false);
            guardar(idFormulario, seccion);
        }
        listaObservacionesBB.get(idFormulario).getMapaSecciones().get(seccion).remove(ObservacionResolucionLicencia);
        reasignarIndice(listaObservacionesBB.get(idFormulario), seccion);
    }
    
    public ObservacionesResolucionLicenciaBean getObservacionesBB() {
        return getObservacionesBB("id");
    }
    
    public ObservacionesResolucionLicenciaBean getObservacionesBB(String idFormulario) {
        if (listaObservacionesBB.containsKey(idFormulario)) {
            return listaObservacionesBB.get(idFormulario);
        }
        return new ObservacionesResolucionLicenciaBean();
    }
    
    public Boolean validarObservaciones(Usuario usuario, Boolean estado, String seccion,String idComponete) {
        List<ObservacionResolucionLicencia> observaciones = this.getObservacionesBB(idComponete).getMapaSecciones().get(seccion);
    ...
    public Boolean validarObservaciones(Usuario usuario, Boolean estado, String seccion) {
        List<ObservacionResolucionLicencia> observaciones = this.getObservacionesBB().getMapaSecciones().get(seccion);

        return validarObservaciones(usuario,  estado, seccion, observaciones );
    }
    
    public Boolean validarObservaciones(Usuario usuario, Boolean estado, String seccion,List<ObservacionResolucionLicencia> observaciones ) {
        List<ObservacionResolucionLicencia> observacionesPendientes = new ArrayList<>();
        if (estado) {
            for (ObservacionResolucionLicencia observacion : observaciones) {
        ...
        if (observacionesPendientes.size() > 0) {
            try {
            	observacionesResolucionLicenciaFacade.guardar(observacionesPendientes);
            } catch (ServiceException e) {
                LOG.error(e);
    ...
    public Boolean esNuevoComentario(ObservacionResolucionLicencia ob) {
    ...
    public void cargarObservacionesHistorial(String idFormulario, String nombreClase, Integer idClase, Date fechaRegistro, String... secciones) {
        try {
			...
			observacion.setListaObservaciones(observacionesResolucionLicenciaFacade.listarHistorialPorIdClaseNombreClase(idClase, nombreClase,fechaRegistro, secciones));
			if (observacion.getListaObservaciones() != null
					&& !observacion.getListaObservaciones().isEmpty()) {
				for (ObservacionResolucionLicencia ob : observacion
    ...
    public void cargarObservacionesHistorialAnterior(String idFormulario, String nombreClase, Integer idClase, Date fechaRegistro, String... secciones) {
        try {
			...
			observacion.setListaObservaciones(observacionesResolucionLicenciaFacade.listarHistorialPorIdClaseNombreClaseHistorico(idClase,nombreClase, fechaRegistro, secciones));
			if (observacion.getListaObservaciones() != null	&& !observacion.getListaObservaciones().isEmpty()) {
				for (ObservacionResolucionLicencia ob : observacion.getListaObservaciones()) {
					if (ob.getIdUsuario().equals(JsfUtil.getLoggedUser().getId())) {
						ob.setDisabled(false);
    ...
    public String obtenerIdSeccionHistorico(String idFormulario) {
        ObservacionesResolucionLicenciaBean observacion = listaObservacionesH.get(idFormulario);
        String sec = "";
        for (String s : observacion.getSeccion()) {
            sec += "-" + s;
        }
