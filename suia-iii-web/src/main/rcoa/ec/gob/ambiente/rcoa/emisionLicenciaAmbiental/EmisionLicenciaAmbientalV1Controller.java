...
@ManagedBean
@ViewScoped
public class EmisionLicenciaAmbientalV1Controller {
	
	private static final Logger LOG = Logger.getLogger(EmisionLicenciaAmbientalV1Controller.class);
    ...
    @Getter
    @Setter
    private List<InstitucionFinanciera> institucionesFinancieras, institucionesFinancierasMaae;
    @Getter
	@Setter
    ...
    @Setter
    @Getter
    private DocumentoResolucionAmbiental documentoPago = new DocumentoResolucionAmbiental();
    @Setter
    @Getter
    ...
    @Getter
	@Setter
	public Boolean cumpleMonto = false, detalleRevisionEmision, calcularValorPagar = false, datosGuardados, persist = false, tieneBypassPPC = false;;
    @Getter
	@Setter
    private String tramite, tecnicoFinanciero, mensaje;
    @Getter
	@Setter
	private Double valorAPagarLicencia, valorAPagarInventario, valorAPagarRgd, montoTotalLicencia, montoTotalInventario, montoTotalRgd;
    
    @Getter
   	@Setter
   	public Boolean cumpleMontoLicencia, cumpleMontoInventario, cumpleMontoRgd, calcularUnoPorMil, tienePagoPorRgd, esEmpresaPublica;

    
    @PostConstruct
	public void init() {
    	try {
    		mensaje = "El pago correspondiente por Licencia Ambiental debe ser depositado en entidades bancarias autorizadas a nombre del Ministerio del Ambiente Agua y Transición Ecológica.";
    		...
    		if (proyectoLicenciaCoa.getAreaResponsable().getTipoEnteAcreditado() != null) {
				if (proyectoLicenciaCoa.getAreaResponsable().getTipoEnteAcreditado().equals("ZONAL")) {
					...
					institucionesFinancieras = institucionFinancieraFacade.obtenerPorNombre(proyectoLicenciaCoa.getAreaResponsable().getAreaName());
					if (institucionesFinancieras == null)
						JsfUtil.addMessageError("No se encontró institución financiara para: " +proyectoLicenciaCoa.getAreaResponsable().getAreaName());
					else
						mensaje = "En cumplimiento al acuerdo ministerial 083-B publicado en el registro oficial edición especial Nro. 387 del 4 de Noviembre del 2015. Para continuar con el proceso de obtención del permiso ambiental, "
								+ "usted debe realizar el pago por concepto de Licencia Ambiental en la entidad financiera o lugar de recaudación correspondiente al " + institucionesFinancieras.get(0).getNombreInstitucion() + " y " 
    		...
    		institucionesFinancierasMaae = institucionFinancieraFacade.obtenerListaInstitucionesFinancierasPC();
    		
    		transaccionesFinancieras = transaccionFinancieraFacade
					.cargarTransaccionesRcoa(proyectoLicenciaCoa
							.getId(), bandejaTareasBean.getTarea()
							.getProcessInstanceId(), bandejaTareasBean
							.getTarea().getTaskId());
    		if(esCorreccion)
    			transaccionesFinancieras = transaccionFinancieraFacade.cargarTransaccionesRcoaPorProceso(proyectoLicenciaCoa.getId(), bandejaTareasBean.getTarea().getProcessInstanceId());
    ...
    public void datosIniciales() {
    	try {
			...
			Double valorPorInventarioForestal = 0.00;
			...
			ProyectoFacilitadorPPC proyectoFacilitadorPPC = proyectoFacilitadorPPCFacade.getByIdProyecto(proyectoLicenciaCoa.getId());
			if(proyectoFacilitadorPPC != null) {
				resolucionAmbiental.setPagoTotalInspeccion(proyectoFacilitadorPPC.getPagoInspeccion());
			}
			...
			cumpleMontoLicencia = false;
			cumpleMontoInventario = false;
			cumpleMontoRgd = false;
    ...
    public void calculaCostoTotal() throws ServiceException {
    	...
    	pagoMinimoProyecto = costoProyecto / 1000;
    	...
    	valorAPagarLicencia = montoTotalProyecto;
    	montoTotalLicencia = valorAPagarLicencia;
    	
    	valorAPagarInventario = resolucionAmbiental.getPagoInventarioForestal();
    	montoTotalInventario = valorAPagarInventario;
    	
    	valorAPagarRgd = resolucionAmbiental.getPagoRGDP();
    	montoTotalRgd = valorAPagarRgd;
				...
				if (monto == 0) {
					...
					if(validarCuenta && cuentaPago.getCuentaNumero() != null && !cuentaPago.getCuentaNumero().equals(nroCuentaTransaccion)) {
						...
						if (monto <= 0) {
							...
							transaccionesFinancieras.add(transaccionFinancieraSelected);
							cumpleMonto = cumpleMonto();
							transaccionFinancieraSelected = new TransaccionFinanciera();
    ...
    public void eliminarComprobante(TransaccionFinanciera item) {
    	transaccionesFinancieras.remove(item);
    	cumpleMonto = cumpleMonto();
		persist = true;
    ...
    private boolean cumpleMonto() {
		...
		for (TransaccionFinanciera transa : transaccionesFinancieras) {
			if(transa.getTipoPago()==null) {//TODO Verificar xq retorna null
		...
		this.montoTotalLicencia = Double.valueOf(decimalValorTramite.format(this.valorAPagarLicencia).replace(",", "."));
		if (montoTotalLicencia >= this.montoTotalLicencia) {
			cumpleMontoLicencia = true;
		} else {
			cumpleMontoLicencia = false;
		}
		
		if(informacionProyectoEia.getValorPorInventarioForestal() != null) {
			this.montoTotalInventario = Double.valueOf(decimalValorTramite.format(this.valorAPagarInventario).replace(",", "."));
			if (montoTotalInventario >= this.montoTotalInventario) {
				cumpleMontoInventario = true;
			} else {
				cumpleMontoInventario = false;
			}
		} else
		...
		if(tienePagoPorRgd) {
			this.montoTotalRgd = Double.valueOf(decimalValorTramite.format(this.valorAPagarRgd).replace(",", "."));
		...
		return cumpleMontoLicencia && cumpleMontoInventario && cumpleMontoRgd;
	}
    ...
    public void generarNUT() throws Exception {
		...
		Tarifas tarifa = tarifasFacade.buscarTarifasPorCodigo(CodigoTasa.EMISION_LICENCIA_AMBIENTAL.getDescripcion());
		if (tarifa == null) {
		...
		numeroUnicoTransaccional.setNutValor(valorAPagarLicencia);
		...
		tarifasNUT.setValorUnitario(valorAPagarLicencia);
		...
		documentoPago.setExtension(".pdf");
    ...
    public Boolean validarDatos() {
    	...
    	if (transaccionesFinancieras == null || transaccionesFinancieras.size() == 0) {
    		JsfUtil.addMessageError("Debe ingresar el número de comprobante de pago");
    		validar = false;
		}
    	if (transaccionesFinancieras != null && transaccionesFinancieras.size() > 0 && !cumpleMonto) {
    ...
    public void guardarDatos() throws Exception {
		...
		if (persist) {
			if (transaccionesFinancieras.size() > 0) {
				...
				for (TransaccionFinanciera tf : transaccionesFinancieras) {
					if(tf.getId() == null)
