...
@ManagedBean
@ViewScoped
public class CargarDiagnosticoAmbientalController implements Serializable{
	...
	@Getter
	@Setter
	private Boolean documentoGuardado, existeNormativaGuias;
	...
	private static final String APLICACION_PDF = "application/pdf";
	...
	public void iniciar() throws JbpmException, NumberFormatException, CmisAlfrescoException
	{
		...
		String obsDiagnostico = (String) variables.get("observacionDiagnosticoAmbiental");		
	...
	public void uploadDiagnostico(FileUploadEvent event) {
		...
		documento.setTipo(APLICACION_PDF);
		...
		listaDocumentosDiagnostico.add(documento);
	...
	public void generarDeclaracion(Boolean marcaAgua) {
		try {
        	...
        	documento.setTipo(APLICACION_PDF);
        	documento.setIdTabla(proyecto.getId());	       		
        	documento.setNombreTabla("DiagnosticoAmbiental");
        	documento.setNombreDocumento(nombreDocumento + ".pdf");
        	
        	
        	documentoCertificacion = documentosFacade.guardarDocumentoAlfresco(proyecto.getCodigoUnicoAmbiental(), "DIAGNOSTICO_AMBIENTAL", 1L, documento, TipoDocumentoSistema.RCOA_CERTIFICADO_INGRESO_DIAGNOSTICO_AMBIENTAL);
	...
	public void uploadListenerDocumentos(FileUploadEvent event) {
		if (documentoDescargado == true) {
			...
			documentoManual.setTipo(APLICACION_PDF);
			documentoManual.setNombreTabla("DiagnosticoAmbiental");
