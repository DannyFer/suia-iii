...
@Stateless
public class LicenciaAmbientalMineriaMunicipal {
	...
	public String visualizarOficio(String codigoProyecto, String resolucion) {
		...
		canton = new UbicacionesGeografica();
		provincia = new UbicacionesGeografica();
		parroquia = new UbicacionesGeografica();
		...
		try {
			...
			proponente = organizacionFacade.buscarPorPersona(proyectoLicenciamientoAmbiental.getUsuario().getPersona(),proyectoLicenciamientoAmbiental.getUsuario().getNombre().toString());
			if (proponente != null) {
...
//			parametros2.put("p_proyecto", proyectoLicenciamientoAmbiental);
		...
		// String Html=null;
		try {
			PlantillaReporte plantillaReporte = this.informeProvincialGADFacade.obtenerPlantillaReporte(TipoDocumentoSistema.TIPO_LICENCIA_MUNICIPAL_MIN.getIdTipoDocumento());
