...
public class UtilGenerarInforme {
    ...
    public static File generarFichero(String cadenaHtml,
        ...
        }else if (entityInforme.getClass().getSimpleName().equals("EntityOficioPronunciamientoEsIA")) {  /// aumentado para los logos del pronunciamiento
        ...
        }else if (entityInforme.getClass().getSimpleName().equals("EntityOficioObsNoSubsanables")) {
        	EntityOficioObsNoSubsanables entidad = (EntityOficioObsNoSubsanables) entityInforme;
            areaRes = entidad.getSiglasEnte();
        }else if (entityInforme.getClass().getSimpleName().equals("EntityInformeOficioDiagnosticoAmbiental")) {
        	EntityInformeOficioDiagnosticoAmbiental entidad = (EntityInformeOficioDiagnosticoAmbiental) entityInforme;
            areaRes = entidad.getSiglasEnte();
        ...
        }else if(entityInforme.getClass().getSimpleName().equals("EntityFirmaResponsabilidadPPC")
        		|| entityInforme.getClass().getSimpleName().equals("OficioResolucionAmbiental")){
        	if(areaRes==null || areasRes!=null){
        		Area areaProyecto=JsfUtil.getBean(ProyectosBean.class).getProyectoRcoa().getAreaResponsable();
        		if(areaProyecto.getTipoArea().getId()==3){
            		areaRes = areaProyecto.getAreaAbbreviation();        			
        		}else {
					areaRes=null;
        ...
        else if(entityInforme.getClass().getSimpleName().contains("PPC"))
        {
        	areaRes = null;
        	try {
        		Field field = entityInforme.getClass().getDeclaredField("siglasEnte");    
        		field.setAccessible(true);
        		Object value = field.get(entityInforme);
        		if(value!=null)
        			areaRes=value.toString();
    ...
    static class HeaderFichaAmbientalMineria extends PdfPageEventHelper {
        ...
        public void onEndPage(PdfWriter writer, Document document) {
            ...
            try {
                ...
                tableHeader.addCell(cellVacia);
                tableHeader.addCell(cellVacia);
                tableHeader.writeSelectedRows(0, -1, 34, 835,
                ...
                tableFooter.writeSelectedRows(0, -36, 0, 40,writer.getDirectContent());
                tableFooter.writeSelectedRows(0, -36, 0, 40,writer.getDirectContent());
    ...
    /**
     * Extrae y convalida los campor a setear en el templare html.
     */

    private static String generarHtmlVue(final String cadenaHtml, final Object entityReporte, String textoNull) {
    	...
    	try {
            ...
            for (Map.Entry<String, Object> m : mapa.entrySet()) {
                buf = buf.replace(m.getKey(), m.getValue() == null ? "<span style='color:red'></span>" : m.getValue().toString());
            }
    ...
    public static File generarFicheroVue(String cadenaHtml,
            final String nombreReporte, final Boolean mostrarNumeracionPagina,
            final Object entityInforme, String textoNull) {
		return generarFicheroVue(cadenaHtml,
		nombreReporte, mostrarNumeracionPagina,
		entityInforme, textoNull, null, false);
	}

    public static File generarFicheroVue(String cadenaHtml, final String nombreReporte, String tipoFormato, final Boolean mostrarNumeracionPagina, final Object entityInforme, String textoNull) {
    	return generarFicheroVue(cadenaHtml, nombreReporte, tipoFormato, mostrarNumeracionPagina, entityInforme, textoNull, null, false);
	}
	...
	public static File generarFicheroVue(String cadenaHtml, final String nombreReporte, String tipoFormato,
		final Boolean mostrarNumeracionPagina, final Object entityReporte, String textoNull, byte[] imagenByte,
		final Boolean mostrarDireccionMae) {
		...
		try {
			cadenaHtml = generarHtmlVue(cadenaHtml, entityReporte, textoNull);

			String buf = cadenaHtml;
			file = File.createTempFile(nombreReporte, "."+tipoFormato);
			List<String> listaTags = new ArrayList<>();
			Pattern pa = Pattern.compile("\\$F[{]\\w+[}$]");
			buf = "<p></p>";
			Matcher mat = pa.matcher(buf);
			...
			writer.setPageEvent(new HeaderFichaAmbientalMineria(null, mostrarNumeracionPagina, mostrarDireccionMae));

			document.open();
			PdfContentByte cb = writer.getDirectContent();
			archivo = nombreReporte + ".pdf";
			createPdfHtml(cadenaHtml, System.getProperty("java.io.tmpdir") + "/" + archivo);
    ...
    public static File generarFicheroVue(String cadenaHtml,
            final String nombreReporte, final Boolean mostrarNumeracionPagina,
            final Object entityInforme) {
		return generarFicheroVue(cadenaHtml, nombreReporte, mostrarNumeracionPagina, entityInforme, "<span style='color:red'> </span>");
	}

    public static File generarFicheroVue(String cadenaHtml, final String nombreReporte, final String tipoFormato, final Boolean mostrarNumeracionPagina, final Object entityInforme) {
		return generarFicheroVue(cadenaHtml, nombreReporte, tipoFormato, mostrarNumeracionPagina, entityInforme, "<span style='color:red'> </span>");
	}
