...
@Stateless
public class LicenciaAmbientalGeneralMunicipal {
	...
	public String visualizarOficio(String codigoProyecto, String resolucion) {
			...
			if (listaOficioAprobacionEias.size() > 0) {
				for (OficioObservacionEia oficioObservacionEia : listaOficioObservacionEias) {
					numeroOficioObservaciones="<tr> <td style=\"font-weight: bold; vertical-align: top;\">Que, </td>"
							+ "<td style=\"text-align: justify;padding-bottom: 10px;vertical-align: top\">mediante Oficio No. "+oficioObservacionEia.getFechaOficioObservaciones()+" de "+oficioObservacionEia.getFechaOficioObservaciones()+" sobre la base del Informe Técnico No. "+numeroOficoInforme+" el Gobierno Autónomo Descentralizado Municipal "+canton+", realiza observaciones al "+nombreProponente+" ubicado en "+provincia+", "+canton+", "+parroquia+";</td>"
							+ "</tr>"
		...
		//String Html=null;
		try {
            PlantillaReporte plantillaReporte = this.informeProvincialGADFacade.obtenerPlantillaReporte(TipoDocumentoSistema.TIPO_LICENCIA_MUNICIPAL_GEN.getIdTipoDocumento());
			...
			entityInforme.setObligacionesPuestas("<li style=\"padding: 5px;text-align: justify;\">"+obligaciones+"</li>");
