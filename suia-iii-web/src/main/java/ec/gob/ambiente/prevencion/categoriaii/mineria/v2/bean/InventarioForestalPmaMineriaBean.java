...
package ec.gob.ambiente.prevencion.categoriaii.mineria.v2.bean;
...
@ManagedBean
@ViewScoped
public class InventarioForestalPmaMineriaBean implements Serializable {

	/**
     *
     */
	private static final long serialVersionUID = 8306276967139328940L;

	private static final Logger LOG = Logger
			.getLogger(InventarioForestalPmaMineriaBean.class);
	...
	@Getter
	@Setter
	private FichaAmbientalMineria fichaAmbiental;
	...
	//MarielaG atributos para historial
	@Getter
	@Setter
	...
	@Getter
	@Setter
	private InventarioForestalPma inventarioForestalPmaOriginal;
	...
	//MarielaG fin atributos historial
	...
	private void cargarDatosIniciales() throws ServiceException {
		...
		fichaAmbiental = fichaAmbientalMineriaFacade.obtenerPorProyecto(proyectoActivo);
		...
		List<InventarioForestalPma> listaInventariosEnBdd = inventarioForestalPmaFacade
															.obtenerAllInventarioForestalPmaMineriaPorFicha(fichaAmbiental.getId());
		...
		if (listaInventariosEnBdd != null) {
			for(InventarioForestalPma inventario : listaInventariosEnBdd){
				if(inventario.getIdRegistroOriginal() == null){
					...
					if(fichaAmbiental.getValidarInventarioForestal() != null && fichaAmbiental.getValidarInventarioForestal() == true){
						descargado = true;
                    }
	...
	public void guardar() {
		try {
            if(inventarioForestalPma.getRemocionVegetal() != null && inventarioForestalPma.getRemocionVegetal()==true) {
                if (getDescargado()) {
                    if (inventarioForestalPma.getInventarioForestal() != null) {
                        inventarioForestalPma.setFichaAmbientalMineria(fichaAmbiental);
                        ...
                        inventarioForestalPmaFacade.guardarMineria(inventarioForestalPma, guardarArchivo);
            ...
            fichaAmbiental.setValidarInventarioForestal(true);
            fichaAmbientalMineriaFacade.guardarFicha(fichaAmbiental);
	...
	//MarielaG fin metodos historial
