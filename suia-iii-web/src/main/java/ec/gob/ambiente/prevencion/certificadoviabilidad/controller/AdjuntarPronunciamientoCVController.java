...
import ec.gob.ambiente.prevencion.certificadoviabilidad.bean.AdjuntarPronunciamientoCVBean;
...
@RequestScoped
@ManagedBean
public class AdjuntarPronunciamientoCVController implements Serializable {
    ...
    @Getter
    @Setter
    @ManagedProperty(value = "#{adjuntarPronunciamientoCVBean}")
    private AdjuntarPronunciamientoCVBean adjuntarPronunciamientoCVBean;
    ...
    private static final Logger LOGGER = Logger.getLogger(AdjuntarPronunciamientoCVController.class);

    public void completarTarea() {
        try {
            //Para los casos que se requieran preguntas del proponente
            if(adjuntarPronunciamientoCVBean.getRespuestasProponente() != null){
                if(!adjuntarPronunciamientoCVBean.getDescargado())
                {
                    JsfUtil.addMessageError("Para continuar debe descargar el documento con las respuestas del proponente.");
                    return;
                }
            }
            if (adjuntarPronunciamientoCVBean.getAdjuntoPronunciamiento() != null) {
                Map<String, Object> params = new ConcurrentHashMap<String, Object>();
                params.put("pronunciamiento", adjuntarPronunciamientoCVBean.getEsPronunciamientoFavorable());
                ...
                certificadoViabilidadAmbientalInterseccionProyectoService.completarTareaYAdjuntarFile(bandejaTareasBean.getTarea()
                        ...
                        adjuntarPronunciamientoCVBean.getAdjuntoPronunciamiento(), proyectosBean.getProyecto(),
                        bandejaTareasBean.getTarea().getProcessInstanceId(), bandejaTareasBean.getTarea().getTaskId(),
                        loginBean.getUsuario(), adjuntarPronunciamientoCVBean.getEsPronunciamientoFavorable());

                if (adjuntarPronunciamientoCVBean.getEsPronunciamientoFavorable()) {
