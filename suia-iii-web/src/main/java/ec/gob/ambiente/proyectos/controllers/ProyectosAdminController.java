...
import ec.gob.ambiente.proyectos.datamodel.LazyProyectCustomDataModel;
...
@ManagedBean
@ViewScoped
public class ProyectosAdminController implements Serializable {
	...
	private void init() {
		try {
			proyectosBean.setProyectosLazy(new LazyProyectCustomDataModel(JsfUtil.getLoggedUser()));
	...
	public String seleccionar(ProyectoCustom proyectoCustom) {
		if(proyectoCustom.getCodigo().contains("-FIS-")){
			proyectoCustom.setSourceType(ProyectoCustom.SOURCE_TYPE_DIGITALIZACION);
		}
    ...
    private Documento guardarDocumento(String codigoProyecto,Documento documento,TipoDocumentoSistema tipoDocumento){
    	try {        	
        	...
        	return documentosFacade.guardarDocumentoAlfrescoSinProyecto(codigoProyecto, Constantes.CARPETA_ARCHIVACION, null, documento, tipoDocumento);
    	} catch (Exception e) {
    		LOG.error(e.getMessage());
    		return null;
    	}    	
